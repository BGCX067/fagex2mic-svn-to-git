...
namespace base
{
	...
	{
#ifdef min
		const int r = StringPiece::wordmemcmp(x.data(), y.data(), min(x.size(), y.size()));
#else
		const int r = StringPiece::wordmemcmp(x.data(), y.data(), std::min(x.size(), y.size()));
#endif
